...
export class DataService{
    ...
    AddNewSerialIn(title: string, description: string, imgUrl: string){
        this.data.push(new Serial(this.data[this.data.length - 1].id + 1, title, description, 1, 1, imgUrl));
        console.table(this.data)
    }
    // OnAddNewSerial(newSerial: {title: string, description: string, seasons: number, episodes: number, imgUrl: string}){
    //     this.data.push({ new Serial(
    //         id: this.data[this.data.length - 1].id + 1,
    //             title: newSerial.title,
    //             description: newSerial.description,
    //             seasons: newSerial.seasons,
    //             episodes: newSerial.episodes,
    //             imgUrl: newSerial.imgUrl
    //     )});
    //   }
